const movies = [{
    ...
    const allMovies = movies.map((item) => {
        return item.poster;                   //put item.title * for (all the titles)
    });
    let imdbLink = movies.map((item) => {
        return item.imdbID;                   
    });
    //console.log(imdbLink)
    /* const filtered = moviesTitle.filter(function (name) {
        return name.includes("Batman")                   //extended syntax
    }); */

    //=====Create Elements=====

    allMovies.forEach(addMoviesToDom);
    //console.log(movies[0].poster);

    function addMoviesToDom(item, index) { 
    const doc = document.createElement("li");
    const container = document.querySelector('#mainMoviesList');
    container.appendChild(doc);
    const linkImdb = document.createElement("a");
    linkImdb.href = innerHTML = ("https://www.imdb.com/title/") + imdbLink[index]
    doc.appendChild(linkImdb);
    const posterImage = document.createElement("img");
    posterImage.src = item;
    linkImdb.appendChild(posterImage);
    //doc.innerHTML = index + " " + item;   //item * (the titles)
    //console.log(container);
    };
    
    //=====Event & Filter=====

    const radioEvent = document.querySelectorAll(".filmFilter");
    const parent = document.getElementById("mainMoviesList");
radioEvent.forEach((btn) => {
        ...
        switch (expr) {
            case 'latest':
                ...
                const latest = function() {
                    latestYearFilter = movies.filter(item => 
                        item.year  >= 2014 
                        ).map((item) => {
                            return item.poster;             
                        });

                        latestImdbFilter = movies.filter(item => //SEGUI QUESTO!!!!
                            item.year  >= 2014 
                            ).map((item) => {
                                return item.imdbID;             
                            });
                            imdbLink = latestImdbFilter;
                      latestYearFilter.forEach(addMoviesToDom);
                      }          
                      latest()
            break;
            case 'avengers':
                ...
                const avengers = function() {
                    avengersPosterFilter = movies.filter(item => 
                        item.title.includes("Avengers")
                      ).map((item) => {
                        return item.poster;             
                    });
                    avengersImdbFilter = movies.filter(item => 
                    item.title.includes("Avengers") 
                            ).map((item) => {
                                return item.imdbID;             
                            });
                            imdbLink = avengersImdbFilter;
                            avengersPosterFilter.forEach(addMoviesToDom);
                };
                avengers();
            break;
            ...
            case 'x-men':
                ...
                const xmen = function() {
                    xmenPosterFilter = movies.filter(
                        item => item.title.includes("X-Men")  
                      ).map((item) => {
                        return item.poster;             
                    });
                    xmenImdbFilter = movies.filter(
                        item => item.title.includes("X-Men")  
                      ).map((item) => {
                        return item.imdbID;             
                    });
                    imdbLink = xmenImdbFilter;
                    xmenPosterFilter.forEach(addMoviesToDom);
                };
                xmen();
            break;
            case 'princess':
                ...
                const princess = function() {
                    princessPosterFilter = movies.filter(
                        item => item.title.includes("Princess")  
                      ).map((item) => {
                        return item.poster;             
                    });
                    princessImdbFilter = movies.filter(
                        item => item.title.includes("Princess")  
                      ).map((item) => {
                        return item.imdbID;             
                    });
                    imdbLink = princessImdbFilter;
                    princessPosterFilter.forEach(addMoviesToDom);
                };
                princess();
            break;
            ...
            case 'batman':
                ...
                const batman = function() {
                    batmanPosterFilter = movies.filter(
                        item => item.title.includes("Batman")  
                      ).map((item) => {
                        return item.poster;             
                    });
                    batmanImdbFilter = movies.filter(
                        item => item.title.includes("Batman")  
                      ).map((item) => {
                        return item.imdbID;             
                    });
                    imdbLink = batmanImdbFilter;
                    batmanPosterFilter.forEach(addMoviesToDom);
                };
                batman();
            break;
            default:
